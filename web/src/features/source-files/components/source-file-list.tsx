...
interface SourceFileListProps {
  ...
  onCopyPrompt: (file: SourceFileDto) => void;
...
function StatusBadge({ status }: { status: SourceFileDto['status'] }) {
  if (status === 'imported') {
    return (
  ...
  if (files.length === 0 && uploadingFiles.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 py-10 text-center">
        <p className="text-sm text-muted-foreground">暂无数据文件，上传后即可交给 AI 导入。</p>
      </div>
    );
  }
