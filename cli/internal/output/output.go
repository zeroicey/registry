...
//   - Table (default): human-readable aligned output with CJK display-width
//     padding (text/tabwriter counts bytes per column, so Chinese headers
//     misalign)
...
//	printer.PrintTable([]string{"ID", "姓名", "编码"}, rows)
//	printer.PrintKeyValue(map[string]string{"ID": "4", "姓名": "张三"})
