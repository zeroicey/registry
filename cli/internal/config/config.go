// Package config manages the Registry CLI configuration file (~/.registry/config.yaml).
//
// Configuration precedence (highest to lowest):
//  1. CLI flags (--baseurl, --token)
//  2. Environment variables (REGISTRY_BASEURL, REGISTRY_TOKEN)
//  3. Config file (~/.registry/config.yaml)
//  4. Default values
package config
...
// default (~/.registry/config.yaml) and REGISTRY_CONFIG_DIR. Pass an empty
...
// Precedence: explicit override (SetPath / --config) > REGISTRY_CONFIG_DIR
// env var > ~/.registry/config.yaml.
func Path() (string, error) {
	if overridePath != "" {
		return overridePath, nil
	}
	if dir := os.Getenv("REGISTRY_CONFIG_DIR"); dir != "" {
	...
	return filepath.Join(home, ".registry", "config.yaml"), nil
}
...
func (c *Config) Resolve(baseURLOverride, tokenOverride string) *Config {
	...
	if v := os.Getenv("REGISTRY_BASEURL"); v != "" {
		resolved.BaseURL = v
	}
